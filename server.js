...
async function sendTelegramMessage(chatId, text) {
    try { await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, { chat_id: chatId, text, parse_mode: 'HTML' }); } catch(e) { console.error('Send message error:', e.message); }
}

async function sendTelegramPhoto(chatId, photoUrl, caption) {
    try { await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`, { chat_id: chatId, photo: photoUrl, caption, parse_mode: 'HTML' }); } catch(e) { console.error('Send photo error:', e.message); }
}
...
app.post('/api/gifts/sell', async (req, res) => {
    ...
    if (price < 10) return res.json({ success: false, error: 'Minimum price 10⭐' });
    try {
...
app.post('/api/nft/withdraw', async (req, res) => {
    ...
    try {
        ...
        if (user.rows[0].stars < 15) throw new Error(`Not enough stars. Need 15⭐`);
        ...
        await pool.query('COMMIT');
        await sendTelegramMessage(telegramId, `✅ Gift withdrawn to your Telegram account! 15⭐ deducted`);
        res.json({ success: true });
    } catch (e) { await pool.query('ROLLBACK'); res.status(400).json({ success: false, error: e.message }); }
});

// ========== АДМИН API С 15 КНОПКАМИ ==========

// 1. Пополнить/снять баланс
app.post('/api/admin/balance', async (req, res) => {
    ...
    await pool.query(`UPDATE users SET stars=stars${operator}$1 WHERE telegram_id=$2`, [amount, target_id]);
    await pool.query(`UPDATE user_finance SET ${action === 'add' ? 'admin_added' : 'admin_removed'} = ${action === 'add' ? 'admin_added+' : 'admin_removed+'}$1 WHERE telegram_id=$2`, [amount, target_id]);
    sendUserBalance(target_id);
    await sendTelegramMessage(target_id, `${action === 'add' ? '➕ Popolnenie' : '➖ Snyatie'}: ${amount}⭐`);
    res.json({ success: true });
});
...
app.post('/api/admin/ban', async (req, res) => {
    ...
    await pool.query("UPDATE users SET banned=$1 WHERE telegram_id=$2", [action === 'ban' ? 1 : 0, target_id]);
    await sendTelegramMessage(target_id, action === 'ban' ? '🔴 You are banned from the bot' : '🟢 You are unbanned');
    res.json({ success: true });
});

// 3. Написать пользователю
app.post('/api/admin/message-user', async (req, res) => {
    if (req.body.admin_id !== ADMIN_ID) return res.sendStatus(403);
    const { target_id, message } = req.body;
    await sendTelegramMessage(target_id, `📩 ADMIN MESSAGE:\n${message}`);
    res.json({ success: true });
});
...
app.post('/api/admin/broadcast', async (req, res) => {
    ...
    let sent = 0;
    for (const user of users.rows) {
        await sendTelegramMessage(user.telegram_id, `📢 BROADCAST:\n${message}`);
        sent++;
...
// 6. Обнулить баланс пользователя
app.post('/api/admin/reset-user-balance', async (req, res) => {
    if (req.body.admin_id !== ADMIN_ID) return res.sendStatus(403);
    const { target_id } = req.body;
    await pool.query("UPDATE users SET stars=0 WHERE telegram_id=$1", [target_id]);
    sendUserBalance(target_id);
    res.json({ success: true });
});

// 7. Сбросить оборот пользователя
app.post('/api/admin/reset-user-turnover', async (req, res) => {
    if (req.body.admin_id !== ADMIN_ID) return res.sendStatus(403);
    const { target_id } = req.body;
    await pool.query("UPDATE users SET turnover=0 WHERE telegram_id=$1", [target_id]);
    res.json({ success: true });
});
...
app.post('/api/admin/get-users', async (req, res) => {
    if (req.body.admin_id !== ADMIN_ID) return res.sendStatus(403);
    const r = await pool.query("SELECT id, telegram_id, name, stars, banned, turnover, games_played, wins FROM users ORDER BY id");
    res.json(r.rows);
});
...
wss.on('connection', ws => {
    ...
    ws.on('close', () => {
        if (rocketState.status === 'waiting') {
            rocketState.bets = rocketState.bets.filter(b => b.telegram_id !== ws.telegram_id);
            broadcast({ type: 'rocket_bets_update', bets: rocketState.bets });
        }
    });
});
    ...
    try {
        ...
        for (const pay of pending.rows) {
            ...
            if (match) {
                ...
                await pool.query("COMMIT");
                await sendTelegramMessage(pay.telegram_id, `✅ Balance replenished! +${pay.stars_amount}⭐`);
