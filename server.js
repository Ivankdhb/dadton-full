...
async function initDatabase() {
    ...
    try {
        ...
        // NFT таблицы
        await client.query(`CREATE TABLE IF NOT EXISTS nft_items (
            id SERIAL PRIMARY KEY, nft_id TEXT UNIQUE, name TEXT, description TEXT, image_url TEXT,
            background_color TEXT DEFAULT '#1a1a1a', pattern TEXT DEFAULT 'classic', rarity TEXT DEFAULT 'COMMON',
            price_stars INTEGER DEFAULT 100, is_limited BOOLEAN DEFAULT false, total_supply INTEGER DEFAULT 1,
            sold_count INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )`);
        await client.query(`CREATE TABLE IF NOT EXISTS user_inventory (
            id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
            nft_id TEXT REFERENCES nft_items(nft_id), purchased_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            is_withdrawn BOOLEAN DEFAULT false, withdrawn_at TIMESTAMP, UNIQUE(user_id, nft_id)
        )`);
        await client.query(`CREATE TABLE IF NOT EXISTS nft_transfers (
            id SERIAL PRIMARY KEY, from_user_id INTEGER, to_user_id INTEGER, nft_id TEXT,
            tx_hash TEXT, status TEXT DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )`);
...
async function sendUserBalance(tgId) {
    ...
    if (r.rows[0]) wss.clients.forEach(c => { if (c.readyState === WebSocket.OPEN && c.telegram_id === tgId) c.send(JSON.stringify({ type: 'balance_update', stars: r.rows[0].stars })); });
}
...
function runRocketLoop() {
    ...
    if (rocketState.status === 'waiting') {
        ...
        if (rocketState.multiplier >= rocketState.crashPoint) {
            ...
            rocketState.bets.forEach(b => { if (!b.cashedOut && b.autoCashout && rocketState.multiplier >= b.autoCashoutValue && !b.cashingOut) { b.cashingOut = true; handleRocketCashout(b.telegram_id, b.autoCashoutValue); } });
...
async function executeRouletteRoll() {
    ...
    setTimeout(async () => {
        ...
        for (let b of rouletteState.bets) if (b.telegram_id !== winner.telegram_id) {
...
app.post('/api/nft/withdraw', async (req, res) => {
    ...
    try {
        await pool.query('BEGIN');
        const user = await pool.query("SELECT id FROM users WHERE telegram_id = $1 FOR UPDATE", [telegramId]);
        if (!user.rows[0]) throw new Error('User not found');
...
app.post('/api/create-invoice', async (req, res) => {
    try {
        const r = await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/createInvoiceLink`, { title: 'Пополнение DadTon', description: `Зачисление ${req.body.amount} звёзд`, payload: `stars_${req.body.telegram_id}_${Date.now()}`, provider_token: "", currency: "XTR", prices: [{ label: `${req.body.amount} Stars`, amount: parseInt(req.body.amount) }] });
...
// ========== АДМИН-ПАНЕЛЬ ==========
app.post('/api/admin/get-users', async (req, res) => { if(req.body.admin_id!==ADMIN_ID) return res.sendStatus(403); const r=await pool.query("SELECT telegram_id,name,stars,banned FROM users LIMIT 50"); res.json(r.rows); });
...
function parseBocBodyPayload(inMsg) {
    try { if(!inMsg?.msg_data?.body) return null; const cell=Cell.fromBase64(inMsg.msg_data.body); const slice=cell.beginParse(); if(slice.remainingBits>=32 && slice.loadUint(32)===0) { const txt=slice.loadStringTail(); const m=txt.match(/deposit:(\d+):(\d+)/); if(m) return { telegram_id: m[1] }; } return null; } catch(e){ return null; }
}
async function checkPendingPayments() {
    try {
        const pending=await pool.query("SELECT * FROM pending_payments WHERE status='pending'"); if(pending.rows.length===0) return;
...
app.get('/icon.png',(req,res)=>{ res.setHeader('Content-Type','image/svg+xml'); res.send('<svg width="256" height="256" viewBox="0 0 256 256"><rect width="256" height="256" fill="#0a0a0a" rx="40"/><circle cx="128" cy="128" r="80" fill="#FFD700"/><text x="128" y="150" font-size="64" text-anchor="middle" fill="#000" font-weight="900" font-family="Arial">D</text></svg>'); });
    ...
    ws.on('message', async msg => {
        try {
            ...
            if (data.type === 'rocket_bet') {
                ...
                if (!u.rows[0] || u.rows[0].stars < data.amount) return ws.send(JSON.stringify({ type: 'err', msg: `Недостаточно баланса. Пополните на ${data.amount - u.rows[0].stars}⭐` }));
            ...
            if (data.type === 'roulette_bet') {
                ...
                if (rouletteState.hasActiveBet[data.telegram_id]) return ws.send(JSON.stringify({ type: 'err', msg: 'Вы уже сделали ставку в этом раунде!' }));
                const u = await pool.query("SELECT stars FROM users WHERE telegram_id=$1", [data.telegram_id]);
                if (!u.rows[0] || u.rows[0].stars < data.amount) return ws.send(JSON.stringify({ type: 'err', msg: `Недостаточно баланса. Пополните на ${data.amount - u.rows[0].stars}⭐` }));
                await pool.query("UPDATE users SET stars=stars-$1 WHERE telegram_id=$2", [data.amount, data.telegram_id]);
