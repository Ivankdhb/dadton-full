...
async function initDatabase() {
    ...
    try {
        ...
        await client.query(`CREATE TABLE IF NOT EXISTS nft_items (
            ...
            rarity TEXT DEFAULT 'COMMON', price_stars INTEGER DEFAULT 100, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )`);
        ...
        await client.query(`CREATE TABLE IF NOT EXISTS market_lots (
            id SERIAL PRIMARY KEY, nft_id TEXT REFERENCES nft_items(nft_id), 
            seller_id INTEGER REFERENCES users(id), price INTEGER NOT NULL, 
            status TEXT DEFAULT 'active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, sold_at TIMESTAMP
        )`);
...
app.get('/api/nft/inventory/:telegramId', async (req, res) => {
    try {
        ...
        if (!user.rows[0]) return res.json({ success: false, inventory: [] });
...
app.post('/api/gifts/sell', async (req, res) => {
    ...
    try {
        ...
        const inventory = await pool.query(
            "SELECT * FROM user_inventory WHERE id = $1 AND user_id = $2 AND is_withdrawn = false",
            [nftId, user.rows[0].id]
        );
        ...
        await pool.query(
            "INSERT INTO market_lots (nft_id, seller_id, price) VALUES ($1, $2, $3)",
            [nft.rows[0].nft_id, user.rows[0].id, price]
        );
