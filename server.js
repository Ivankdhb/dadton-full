...
let botPaused = false, rocketState = { 
    status: 'waiting', 
    multiplier: 1.00, 
    crashPoint: 1.00, 
    timer: 10, 
...
async function executeRouletteRoll() {
    ...
    setTimeout(async () => {
        await pool.query("UPDATE users SET stars=stars+$1, wins=wins+1, games_played=games_played+1, turnover=turnover+$2 WHERE telegram_id=$3", 
            [rouletteState.totalBank, winner.amount, winner.telegram_id]);
        await saveGameHistory(winner.telegram_id, 'roulette', 'Рулетка', winner.amount, rouletteState.totalBank, 
            rouletteState.totalBank - winner.amount, rouletteState.totalBank / winner.amount);
        for (let b of rouletteState.bets) {
...
app.post('/api/register', async (req, res) => {
    ...
    const r = await pool.query(
        "INSERT INTO users (telegram_id, name, avatar, username, referrer_id) VALUES ($1,$2,$3,$4,$5) RETURNING *", 
    ...
    ws.on('message', async msg => {
        try {
            ...
            if (data.type === 'rocket_bet') {
                ...
                if (!user.rows[0] || user.rows[0].stars < data.amount) return;
